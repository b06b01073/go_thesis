...
def compute_invalid_moves(state, player, ko_protect=None):
    ...
    return invalid_moves > 0 #binary plane
...
def adj_data(state, action2d, player):
    neighbors = neighbor_deltas + action2d # up, down, left, right offset + base action coord, a list of coord
    valid = (neighbors >= 0) & (neighbors < state.shape[1]) # make sure neighbors is in the board
